...
import type { WebcamPreviewState } from "./ipc/handlers";
...
contextBridge.exposeInMainWorld("electronAPI", {
	...
	setWebcamPreviewState: (state: Partial<WebcamPreviewState>) => {
		return ipcRenderer.invoke("set-webcam-preview-state", state);
	},
	openWebcamPreview: () => {
		return ipcRenderer.invoke("open-webcam-preview");
	},
	closeWebcamPreview: () => {
		return ipcRenderer.invoke("close-webcam-preview");
	},
	requestWebcamEnabled: (enabled: boolean) => {
		return ipcRenderer.invoke("request-webcam-enabled", enabled);
	},
	getWebcamPreviewState: () => {
		return ipcRenderer.invoke("get-webcam-preview-state");
	},
	onWebcamEnabledRequest: (callback: (enabled: boolean) => void) => {
		const listener = (_event: unknown, enabled: boolean) => callback(enabled);
		ipcRenderer.on("webcam-enabled-request", listener);
		return () => ipcRenderer.removeListener("webcam-enabled-request", listener);
	},
	onWebcamPreviewState: (callback: (state: WebcamPreviewState) => void) => {
		const listener = (_event: unknown, state: WebcamPreviewState) => callback(state);
		ipcRenderer.on("webcam-preview-state", listener);
		return () => ipcRenderer.removeListener("webcam-preview-state", listener);
	},
