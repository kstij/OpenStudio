...
const DEFAULT_WEBCAM_PREVIEW_STATE: WebcamPreviewState = {
	recording: false,
	webcamEnabled: false,
...
let webcamPreviewState: WebcamPreviewState = { ...DEFAULT_WEBCAM_PREVIEW_STATE };
...
export function registerIpcHandlers(
	...
	getWebcamPreviewWindow: () => BrowserWindow | null,
	onRecordingStateChange?: (recording: boolean, sourceName: string) => void,
	switchToHud?: () => void,
) {
	const applyWebcamPreviewState = (partial: Partial<WebcamPreviewState>) => {
		webcamPreviewState = {
			...webcamPreviewState,
			...partial,
			blur: clamp(Number(partial.blur ?? webcamPreviewState.blur), 0, 20),
		};

		// Preview visibility is camera-toggle driven, independent of recording start.
		const shouldShow = webcamPreviewState.webcamEnabled;
		const existing = getWebcamPreviewWindow();
		...
		if (!existing && previewWindow.webContents) {
			...
			previewWindow.webContents.on("unresponsive", () => {
				console.error("[webcam-preview] render process unresponsive");
			});
		}
		...
		if (previewWindow.webContents.isLoading()) {
			previewWindow.webContents.once("did-finish-load", () => {
				if (!previewWindow.isDestroyed()) {
					previewWindow.webContents.send("webcam-preview-state", webcamPreviewState);
				}
			});
		} else {
			previewWindow.webContents.send("webcam-preview-state", webcamPreviewState);
	...
	const broadcastWebcamEnabledRequest = (enabled: boolean) => {
		const mainWindow = getMainWindow();
		if (mainWindow && !mainWindow.isDestroyed()) {
			mainWindow.webContents.send("webcam-enabled-request", enabled);
		}
	};
	...
	ipcMain.handle("set-webcam-preview-state", (_, state: Partial<WebcamPreviewState>) => {
		applyWebcamPreviewState(state);
		return { success: true };
	});

	ipcMain.handle("open-webcam-preview", () => {
		applyWebcamPreviewState({ webcamEnabled: true });
		return { success: true };
	});

	ipcMain.handle("close-webcam-preview", () => {
		applyWebcamPreviewState({ webcamEnabled: false });
		return { success: true };
	});

	ipcMain.handle("request-webcam-enabled", (_, enabled: boolean) => {
		applyWebcamPreviewState({ webcamEnabled: enabled });
		broadcastWebcamEnabledRequest(enabled);
		return { success: true };
	});

	ipcMain.handle("get-webcam-preview-state", () => {
		return webcamPreviewState;
	});
	...
	ipcMain.handle("set-recording-state", (_, recording: boolean) => {
		...
		const previewWindow = getWebcamPreviewWindow();
		if (previewWindow && !previewWindow.isDestroyed() && !previewWindow.webContents.isLoading()) {
			previewWindow.webContents.send("webcam-preview-state", webcamPreviewState);
		}
	});
		...
		async (_, projectData: unknown, suggestedName?: string, existingProjectPath?: string) => {
			try {
				...
				const result = await dialog.showSaveDialog({
					...
					filters: [
						{
							name: mainT("dialogs", "fileDialogs.openstudioProject"),
