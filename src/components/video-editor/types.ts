...
export type ScreenLayoutPreset = "full-screen" | "comfortable" | "padded" | "custom";

export const DEFAULT_SCREEN_LAYOUT_PRESET: ScreenLayoutPreset = "full-screen";

export const SCREEN_LAYOUT_PRESET_PADDING: Record<ScreenLayoutPreset, number> = {
	"full-screen": 0,
	comfortable: 40,
	padded: 50,
	custom: 60,
...
export function resolveScreenPadding(preset: ScreenLayoutPreset, paddedValue: number): number {
	if (preset === "padded" || preset === "custom") {
		return paddedValue;
	}

	return SCREEN_LAYOUT_PRESET_PADDING[preset];
}
...
export const DEFAULT_WEBCAM_MIRRORED = true;

/** Webcam blur strength in pixels (0-20). */
export type WebcamBackgroundBlur = number;

export const DEFAULT_WEBCAM_BACKGROUND_BLUR: WebcamBackgroundBlur = 0;
