	...
	return {
		...
		webcamMirrored:
			typeof (editor as { webcamMirrored?: unknown }).webcamMirrored === "boolean"
				? Boolean((editor as { webcamMirrored?: boolean }).webcamMirrored)
				: DEFAULT_WEBCAM_MIRRORED,
		webcamBackgroundBlur: isFiniteNumber(
			(editor as { webcamBackgroundBlur?: unknown }).webcamBackgroundBlur,
		)
			? clamp((editor as { webcamBackgroundBlur?: number }).webcamBackgroundBlur ?? 0, 0, 20)
			: DEFAULT_WEBCAM_BACKGROUND_BLUR,
		exportQuality:
