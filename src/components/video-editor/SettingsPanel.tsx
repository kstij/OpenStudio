...
interface SettingsPanelProps {
	...
	onScreenLayoutPresetChange?: (preset: ScreenLayoutPreset) => void;
	...
	onWebcamBackgroundBlurChange?: (blur: number) => void;
	onWebcamBackgroundBlurCommit?: () => void;
}
...
export function SettingsPanel({
	...
	onWebcamBackgroundBlurChange,
	onWebcamBackgroundBlurCommit,
}: SettingsPanelProps) {
	...
	const webcamSizeQuickPresets: Array<{ label: string; value: WebcamSizePreset }> = [
		{ label: "S", value: 18 },
		{ label: "M", value: 25 },
		{ label: "L", value: 34 },
	];
			...
			<div className="flex-1 overflow-y-auto custom-scrollbar p-4 pb-0">
				...
				>
						...
						>
							...
							<AccordionContent className="pb-3">
								...
								{webcamLayoutPreset === "picture-in-picture" && (
									<div className="p-2 rounded-lg bg-white/5 border border-white/5 mt-2">
										...
										<div className="mt-2 grid grid-cols-3 gap-1.5">
											{webcamSizeQuickPresets.map((preset) => (
												<Button
													key={preset.label}
													type="button"
													onClick={() => {
								...
								{webcamLayoutPreset === "picture-in-picture" && (
									<div className="p-2 rounded-lg bg-white/5 border border-white/5 mt-2">
										<div className="flex items-center justify-between mb-1.5">
											<div className="text-[10px] font-medium text-slate-300">Mirror</div>
											<Switch
								...
								{webcamLayoutPreset === "picture-in-picture" && (
									<div className="p-2 rounded-lg bg-white/5 border border-white/5 mt-2">
										<div className="flex items-center justify-between mb-1.5">
											<div className="text-[10px] font-medium text-slate-300">Webcam Blur</div>
											<div className="text-[10px] font-medium text-slate-400">
												{webcamBackgroundBlur.toFixed(1)}px
											</div>
										</div>
										<Slider
											value={[webcamBackgroundBlur]}
											onValueChange={(values) => onWebcamBackgroundBlurChange?.(values[0])}
											onValueCommit={() => onWebcamBackgroundBlurCommit?.()}
											min={0}
											max={20}
											step={0.5}
											className="w-full"
					...
					<AccordionItem value="effects" className="border-white/5 rounded-xl bg-white/[0.02] px-3">
						...
						<AccordionContent className="pb-3">
							<div className="mb-3">
								<div className="text-[10px] font-medium text-slate-300 mb-2">Layout</div>
								<div className="grid grid-cols-4 gap-2">
									{screenLayoutPresets.map((preset) => {
										const active = screenLayoutPreset === preset.value;
										return (
											<button
												key={preset.value}
												type="button"
												onClick={() => onScreenLayoutPresetChange?.(preset.value)}
												className={cn(
