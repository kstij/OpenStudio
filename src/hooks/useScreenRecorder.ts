...
const IDEAL_CAPTURE_WIDTH = 2560;
const IDEAL_CAPTURE_HEIGHT = 1600;
...
type WebcamPreviewShape = "rectangle" | "rounded" | "circle" | "square";
...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const [webcamPreviewBlur, setWebcamPreviewBlur] = useState(initialPrefs.webcamBlur);
	...
	useEffect(() => {
		if (!window.electronAPI?.onWebcamEnabledRequest) {
			return;
		}

		const cleanup = window.electronAPI.onWebcamEnabledRequest((enabled) => {
			void setWebcamEnabled(enabled);
		});
	...
	const startRecording = async () => {
		try {
			...
			const nativeCaptureWidth = Math.max(
				IDEAL_CAPTURE_WIDTH,
				Math.floor(
			...
			try {
				await videoTrack.applyConstraints({
					...
					width: { ideal: nativeCaptureWidth, max: TARGET_WIDTH },
					height: { ideal: nativeCaptureHeight, max: TARGET_HEIGHT },
				});
			} catch (constraintError) {
				console.warn(
					"Unable to lock high-quality capture constraints, using best available track settings.",
					constraintError,
				);
			}
	...
	useEffect(() => {
		const onStorage = (event: StorageEvent) => {
			if (event.key !== "openstudio_user_preferences") return;
			const latest = loadUserPreferences();
			setWebcamPreviewMirrored(latest.webcamMirrored);
			setWebcamPreviewShape(latest.webcamShape);
			setWebcamPreviewBlur(latest.webcamBlur);
		};
	...
	useEffect(() => {
		void window.electronAPI
			.setWebcamPreviewState({
				webcamEnabled,
				webcamDeviceId,
				mirrored: webcamPreviewMirrored,
				shape: webcamPreviewShape,
				blur: webcamPreviewBlur,
			})
			.catch((error) => {
