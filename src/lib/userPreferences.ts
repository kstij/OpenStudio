...
const PREFS_KEY = "openstudio_user_preferences";
...
export interface UserPreferences {
	...
	/** Default webcam mirror setting */
	webcamMirrored: boolean;
	/** Default webcam shape in PiP mode */
	webcamShape: "rectangle" | "rounded" | "circle" | "square";
	/** Default webcam blur strength */
	webcamBlur: number;
}
...
export function loadUserPreferences(): UserPreferences {
	...
	return {
		...
		webcamBlur:
			typeof raw.webcamBlur === "number" && Number.isFinite(raw.webcamBlur)
				? Math.max(0, Math.min(20, raw.webcamBlur))
				: DEFAULT_PREFS.webcamBlur,
	};
}
