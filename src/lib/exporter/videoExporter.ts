...
const MIN_MP4_EXPORT_BITRATE = 8_000_000;
...
export class VideoExporter {
	...
	private async exportWithEncoderPreference(
		...
		try {
			...
			this.outputWidth = this.resolveOutputDimension(this.config.width, videoInfo.width);
			this.outputHeight = this.resolveOutputDimension(this.config.height, videoInfo.height);
			...
			const renderer = new FrameRenderer({
				...
				screenLayoutPreset: this.config.screenLayoutPreset,
				...
				webcamBackgroundBlur: this.config.webcamBackgroundBlur,
	...
	private async initializeEncoder(hardwareAcceleration: HardwareAcceleration): Promise<void> {
						...
						if (isFirstChunk && this.videoDescription) {
							...
							const metadata: EncodedVideoChunkMetadata = {
								decoderConfig: {
									...
									codedWidth: this.outputWidth || this.config.width,
									codedHeight: this.outputHeight || this.config.height,
		...
		const encoderConfig: VideoEncoderConfig = {
			codec: this.config.codec || "avc1.640033",
			width: this.outputWidth || this.config.width,
			height: this.outputHeight || this.config.height,
			bitrate: Math.max(this.config.bitrate, MIN_MP4_EXPORT_BITRATE),
	...
	private resolveOutputDimension(requested: number, source: number): number {
		const fallback = Number.isFinite(source) && source > 0 ? source : 1920;
		const candidate = Number.isFinite(requested) && requested > 0 ? requested : fallback;
		const even = Math.floor(candidate / 2) * 2;
