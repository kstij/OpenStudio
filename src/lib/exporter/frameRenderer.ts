...
export class FrameRenderer {
	...
	constructor(config: FrameRenderConfig) {
		const normalizedWidth =
			Number.isFinite(config.width) && config.width > 0
				? Math.max(2, Math.floor(config.width / 2) * 2)
				: Math.max(2, Math.floor(config.videoWidth / 2) * 2);
		const normalizedHeight =
			Number.isFinite(config.height) && config.height > 0
				? Math.max(2, Math.floor(config.height / 2) * 2)
				: Math.max(2, Math.floor(config.videoHeight / 2) * 2);
	...
	async initialize(): Promise<void> {
		...
		this.applyHighQualitySmoothing(this.compositeCtx);
		...
		this.applyHighQualitySmoothing(this.rasterCtx);
		...
		if (this.config.showShadow) {
			...
			this.applyHighQualitySmoothing(this.shadowCtx);
		}
	...
	private async setupBackground(): Promise<void> {
		...
		this.applyHighQualitySmoothing(bgCtx);
	...
	private updateLayout(webcamFrame?: VideoFrame | null): void {
		...
		// Padding is treated as px from preset/custom value.
		// Vertical stack ignores padding — it's full-bleed
		const screenLayoutPreset = this.config.screenLayoutPreset ?? "full-screen";
		...
		this.layoutCache = {
			...
			baseScale: scale,
			baseOffset: { x: screenRect.x, y: screenRect.y },
			maskRect: screenRect,
			screenCornerRadius: scaledBorderRadius,
	...
	private applyHighQualitySmoothing(ctx: CanvasRenderingContext2D): void {
	...
	private compositeWithShadows(webcamFrame?: VideoFrame | null): void {
		...
		this.applyHighQualitySmoothing(ctx);
		...
		const screenLayoutPreset = this.config.screenLayoutPreset ?? "full-screen";
		const resolvedPadding = resolveScreenPadding(screenLayoutPreset, this.config.padding ?? 0);
		const skipBackground =
			screenLayoutPreset === "full-screen" ||
			(screenLayoutPreset === "custom" && resolvedPadding <= 0);
		if (!skipBackground && this.backgroundSprite) {
		...
		const screenMask = this.layoutCache?.maskRect ?? { x: 0, y: 0, width: w, height: h };
		const screenCornerRadius = this.layoutCache?.screenCornerRadius ?? 0;
		...
		if (
			...
			this.applyHighQualitySmoothing(shadowCtx);
		...
		if (webcamFrame && webcamRect) {
			...
			const webcamMirrored = this.config.webcamMirrored ?? true;
			const webcamBlur = this.config.webcamBackgroundBlur ?? 0;
			ctx.save();
			drawCanvasClipPath(
				ctx,
				webcamRect.x,
			...
			ctx.fill();
			ctx.clip();
			ctx.filter = webcamBlur > 0 ? `blur(${webcamBlur}px)` : "none";
			if (webcamMirrored) {
				ctx.translate(webcamRect.x + webcamRect.width, webcamRect.y);
				ctx.scale(-1, 1);
				ctx.drawImage(
